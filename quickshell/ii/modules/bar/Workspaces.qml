...
Item {
    ...
    // Function to update dynamic workspaces list
    function updateDynamicWorkspaces() {
        // Track the highest workspace visited, even if empty
        var currentWorkspaceId = monitor?.activeWorkspace?.id;
        if (currentWorkspaceId && currentWorkspaceId > 0) {
            if (currentWorkspaceId > maxVisitedWorkspaceId) {
                maxVisitedWorkspaceId = currentWorkspaceId;
            }
        }
        
        // Build a continuous list from 1..maxVisited
        var newWorkspaces = [];
        for (var i = 1; i <= maxVisitedWorkspaceId; i++) {
        ...
        // console.log("Current workspace:", currentWorkspaceId, "| Max visited:", maxVisitedWorkspaceId, "| Dynamic workspaces:", JSON.stringify(newWorkspaces));
    }

    // Initialize dynamic workspaces when the component is created
    Component.onCompleted: updateDynamicWorkspaces()

    // Listen for changes in Hyprland workspaces
    Connections {
        target: Hyprland.workspaces
        function onValuesChanged() {
            updateDynamicWorkspaces();
        }
    }
    ...
    // Timer to ensure we catch workspace changes that might be missed
    Timer {
    ...
    RowLayout {
        ...
        Repeater {
            ...
            Rectangle {
                ...
                property int workspaceId: root.dynamicWorkspaces[index]
                property int currentIndex: index
                // Highlight chain segments only for occupied neighbors
                property bool isCurrentOccupied: Hyprland.workspaces.values.some(ws => ws.id === workspaceId)
                property bool leftOccupied: currentIndex > 0 && Hyprland.workspaces.values.some(ws => ws.id === root.dynamicWorkspaces[currentIndex - 1])
                property bool rightOccupied: currentIndex < (root.dynamicWorkspaces.length - 1) && Hyprland.workspaces.values.some(ws => ws.id === root.dynamicWorkspaces[currentIndex + 1])
    ...
    Rectangle {
        ...
        property int activeIndex: {
            for (var i = 0; i < root.dynamicWorkspaces.length; i++) {
                if (root.dynamicWorkspaces[i] === monitor?.activeWorkspace?.id) {
                    return i;
                }
            }
            return -1;
    ...
    // Workspaces - numbers and interactions
    RowLayout {
        id: rowLayoutNumbers
        z: 3
        ...
        Repeater {
            ...
            Button {
                id: button
                property int workspaceValue: root.dynamicWorkspaces[index]
                ...
                background: Item {
                    ...
                    StyledText { // Workspace number text
                        ...
                        color: (monitor?.activeWorkspace?.id == button.workspaceValue) ? 
                            ...
                            (Hyprland.workspaces.values.some(ws => ws.id === button.workspaceValue) ? Appearance.m3colors.m3onSecondaryContainer : Appearance.colors.colOnLayer1Inactive)
